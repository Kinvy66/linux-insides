...
The [compiler](https://en.wikipedia.org/wiki/GNU\_Compiler\_Collection) will not show us warning that `a` field is not uninitialized. But if we will run this program with [valgrind](https://en.wikipedia.org/wiki/Valgrind) tool, we will see the following output:
...
![kernel configuration menu](../../.gitbook/assets/kernel\_configuration\_menu1.png)
...
or in other words somebody wants to access a [page](https://en.wikipedia.org/wiki/Page\_\(computer\_memory\)), a [page fault](https://en.wikipedia.org/wiki/Page\_fault) exception is generated. This is achieved by the fact that the `kmemcheck` marks memory pages as `non-present` (more about this you can read in the special part which is devoted to [Paging](https://0xax.gitbook.io/linux-insides/summary/theory/linux-theory-1)). If a `page fault` exception occurred, the exception handler knows about it and in a case when the `kmemcheck` is enabled it transfers control to it. After the `kmemcheck` will finish its checks, the page will be marked as `present` and the interrupted code will be able to continue execution. There is little subtlety in this chain. When the first instruction of interrupted code will be executed, the `kmemcheck` will mark the page as `non-present` again. In this way next access to memory will be caught again.
...
![kernel configuration menu](../../.gitbook/assets/kernel\_configuration\_menu2.png)
...
First of all the `kmemcheck_fault` function checks that the fault occurred by the correct reason. At first we check the [flags register](https://en.wikipedia.org/wiki/FLAGS\_register) and check that we are in normal kernel mode:

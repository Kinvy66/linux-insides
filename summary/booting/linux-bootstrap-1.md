...
We get `0xfffffff0`, which is 16 bytes below 4GB. This point is called the [reset vector](https://en.wikipedia.org/wiki/Reset\_vector). It's the memory location at which the CPU expects to find the first instruction to execute after reset. It contains a [jump](https://en.wikipedia.org/wiki/JMP\_\(x86\_instruction\)) (`jmp`) instruction that usually points to the [BIOS](https://en.wikipedia.org/wiki/BIOS) (Basic Input/Output System) entry point. For example, if we look in the [coreboot](https://www.coreboot.org/) source code ([src/cpu/x86/16bit/reset16.inc](https://review.coreboot.org/plugins/gitiles/coreboot/+/refs/heads/4.11\_branch/src/cpu/x86/16bit/reset16.inc)), we see:
...
We also see that the `reset` section is `16` bytes and is compiled to start from the address `0xfffffff0` ([src/cpu/x86/16bit/reset16.ld](https://review.coreboot.org/plugins/gitiles/coreboot/+/refs/heads/4.11\_branch/src/cpu/x86/16bit/reset16.ld)):
...
![Simple bootloader which prints only !](../../.gitbook/assets/simple\_bootloader.png)
...
![kernel first address](../../.gitbook/assets/kernel\_first\_address.png)
...
![Try vmlinuz in qemu](../../.gitbook/assets/try\_vmlinuz\_in\_qemu.png)
...
![stack](../../.gitbook/assets/stack1.png)
